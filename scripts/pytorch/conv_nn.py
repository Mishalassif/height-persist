...
class ConvNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(40, 20, kernel_size=4, padding=2),
            nn.MaxPool2d(kernel_size = 2),
            nn.ReLU(),
            #nn.Dropout(p=0.2),
            nn.Conv2d(20, 10, kernel_size=4, padding=2),
            nn.MaxPool2d(kernel_size = 2),
            nn.ReLU(),
            #nn.Dropout(p=0.2)  ,
            nn.Conv2d(10, 5, kernel_size=4, padding=2),
            nn.MaxPool2d(kernel_size = 2),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(45, 20),
...
for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
    ...
    testloader = torch.utils.data.DataLoader(
                      dataset,
                      batch_size=5*BATCH_SIZE, sampler=test_subsampler)

    network = ConvNet()
    network = network.double()
    network = network.to(device)
    network.apply(reset_weights)
    print("Number of parameters: " + str(count_parameters(network)))
    ...
    for epoch in range(0, num_epochs):
      ...
      if epoch % 5 == 4 or 1:
        ...
        #if i % 10 == 9:
        print('Accuracy after epoch ' + str(epoch) + ': ' +str(100.0*(correct / total)))
